...
from evaluation.datasets.uav123dataset import UAV123


class ExperimentUAV123(object):
    ...
    def __init__(self, root_dir, result_dir='results', report_dir='reports'):
        self.dataset = UAV123(root_dir)
        self.result_dir = os.path.join(result_dir, 'uav123')
        self.report_dir = os.path.join(report_dir, 'uav123')
    ...
    def report(self, tracker_names,dataset,epoch):
        ...
        for name in tracker_names:
            ...
            if dataset not in performance:
                performance.update({dataset : {}})
            performance[dataset].update({'epoch{}'.format(epoch): {}})

            for s, anno in enumerate(self.dataset):
                seq_name = self.dataset.seq_names[s]
                record_file = os.path.join(self.result_dir,'uav_%s.txt' % seq_name)
                ...
                time_file = os.path.join(self.result_dir,'times/uav_%s_time.txt' % seq_name)
            ...
            performance[dataset]['epoch{}'.format(epoch)].update({
                'success_score':
                    succ_score,
                'precision_score':
                    prec_score,
